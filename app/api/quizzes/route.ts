...
// GET /api/quizzes?status=...&categoryId=...&difficulty=...
export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const queryString = searchParams.toString();
    const path = queryString ? `/quizzes?${queryString}` : '/quizzes';
    
    const result = await forwardRequest(path, request, { method: 'GET' });
    return NextResponse.json(result.body, { status: result.status });
  } catch (err: any) {
    console.error('Error proxying GET /api/quizzes', err);
...
export async function POST(request: Request) {
  try {
    ...
    const result = await forwardRequest('/quizzes', request, opts);
    return NextResponse.json(result.body, { status: result.status });
  } catch (err: any) {
    console.error('Error proxying POST /api/quizzes', err);
    return NextResponse.json({ message: 'Lỗi server', error: err?.message || String(err) }, { status: 500 });
  }
}
